...
export default function HomePage() {
  ...
  const loadRobloxConfig = async () => {
    try {
      ...
      const { robloxClientId, robloxClientSecret, oauthOnlyLogin } =
        response.data;
      const currentOrigin =
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Manage your Roblox group workspaces
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            {isOwner && (
              <Button
                ...
                classoverride="flex items-center gap-2"
              >
            ...
            <Button
              onClick={checkRoles}
              classoverride="flex items-center gap-2 bg-zinc-200 hover:bg-zinc-300 text-zinc-800 dark:bg-zinc-700 dark:hover:bg-zinc-600 dark:text-white"
            >
            ...
            {isOwner && (
              <Button
                ...
                classoverride="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white"
              >
        ...
        {login.workspaces?.length ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 sm:gap-6">
            {login.workspaces.map((workspace, i) => (
              <div
                key={i}
                className="group bg-white dark:bg-zinc-800 rounded-xl shadow-sm overflow-hidden transition-all duration-200 hover:shadow-lg hover:scale-[1.02] cursor-pointer aspect-square flex flex-col border border-zinc-200 dark:border-zinc-700"
                onClick={() => gotoWorkspace(workspace.groupId)}
              >
                <div
                  className="flex-1 bg-cover bg-center transition-transform duration-200 group-hover:scale-105"
                  style={{ backgroundImage: `url(${workspace.groupThumbnail})` }}
                />
                <div className="p-4 flex items-center justify-between bg-white dark:bg-zinc-800">
                  <h3 className="text-base sm:text-lg font-semibold text-zinc-900 dark:text-white truncate flex-1">
                    {workspace.groupName}
                  </h3>
                  <IconChevronRight className="h-5 w-5 text-zinc-400 dark:text-zinc-500 flex-shrink-0 ml-2 group-hover:text-primary transition-colors" />
                </div>
              </div>
            ))}
          </div>
        ) : (
          <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-8 sm:p-12 flex flex-col items-center justify-center text-center border border-zinc-200 dark:border-zinc-700">
            <div className="bg-zinc-100 dark:bg-zinc-700 rounded-full p-4 mb-6">
              <IconBuildingSkyscraper className="h-12 w-12 text-zinc-400 dark:text-zinc-500" />
            </div>
            <h3 className="text-xl sm:text-2xl font-semibold text-zinc-900 dark:text-white mb-2">
            ...
            {isOwner ? (
              <Button onClick={() => setIsOpen(true)} classoverride="flex items-center gap-2">
                <IconPlus className="h-5 w-5" />
                Create Workspace
              </Button>
            ) : (
          ...
          >
            ...
            <div className="fixed inset-0 overflow-y-auto">
              <div className="flex min-h-full items-center justify-center p-4 text-center">
                ...
                >
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-white dark:bg-zinc-800 p-6 text-left align-middle shadow-xl transition-all border border-zinc-200 dark:border-zinc-700">
                    <Dialog.Title
                      as="h3"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                            />
                          </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                        ...
                        <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 ml-6">
                          When enabled, users will only see the Roblox OAuth login
                          button.
                        </p>
                      </div>
                    </div>
